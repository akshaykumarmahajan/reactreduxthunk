...
function DataTable(){
  ...
  const toggleDetails = (index) => {
    ...
    console.log("position",position,details.slice())
    let newDetails = details.slice()
    
    console.log("newDetails",newDetails)
    if (position !== -1) {
      newDetails.splice(position, 1)
      console.log("inif",newDetails.splice(position, 1))
    } else {
      newDetails = [...details, index]
      console.log("else",newDetails,index)
    }
            ...
            (item, index)=>{
              ...
              <CCollapse show={details.includes(index)}>
                <CCardBody>
                  ...
                  <p className="text-muted">User since: {item.registered}{details}</p>
